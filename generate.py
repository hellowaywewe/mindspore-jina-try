...
def load_minst(dir_path, kind='train'):
...
def save_mnist_to_jpg(images, labels, save_dir, kind, num):
    ...
    for i in range(num):
        ...
        cv2.imwrite(jpg_name, img_np)
        print('{} ==> {}_{}_{}.jpg'.format(i, kind, i, label_val))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="MNIST Dataset Operations")
    parser.add_argument('--data_dir', type=str, default='/tmp/jina/mnist/train', help='MNIST dataset dir')
    parser.add_argument('--kind', type=str, default='train', help='MNIST dataset: train or t10k')
    parser.add_argument('--save_dir', type=str, default='/tmp/jina/mnist/train/jpg', help='used to save mnist jpg')
    args = parser.parse_args()

    if not os.path.exists(args.data_dir):
        os.makedirs(args.data_dir)

    images_np, labels_np, kind_num = load_minst(args.data_dir, args.kind)
